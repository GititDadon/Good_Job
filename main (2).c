...
void read_Employee_Data_File(struct Employee* employee[100])
{
	...
	for (i = 0; i < 3; ++i)// cannot run until 'size'->??
	{
...
void print_data_file_employer(struct Employer emp)
{
	...
	FILE* fp = fopen("C:\\Users\\Administrator\\source\\repos\\good job\\Employer.txt", "w");
	if (!fp)
		exit(-1);
	fprintf(fp, "%s", emp.first_name);
	fprintf(fp, "%s", emp.last_name);
	fprintf(fp, "%s", emp.gender);
	fprintf(fp, "%s", emp.id);
	fprintf(fp, "%d", emp.age);
	fprintf(fp, "%s", emp.phone_number);
	fprintf(fp, "%d", emp.password);
	fprintf(fp, "%s", emp.Email);
	fprintf(fp, "%d", emp.policies);
...
void init_file_employee(struct Employee arr[])
{
	int i;
	FILE* fp = fopen("C:\\Users\\Administrator\\source\\repos\\good job\\Employer.txt", "w");
	for (i = 0; i < 3; i++)
...
void logIn(struct Employee workers[3], struct Employer bosses[3])//המשתמש נכנס למערכת
{
	int numOfBosses = 3;
	int numOfWorkers = 3;
	char id[9];
...
void read_Employer_Data_File(struct Employer* employer)
{
	int i = 0;
